...
ConnectionOptions ConnectionOptions::_parse_uri(const std::string &uri) const {
    ...
    std::string szAuth;
    std::string szPath;
    std::tie(szType, szAuth, szPath) = _split_uri(uri);

    ConnectionOptions opts;

    _set_auth_opts(szAuth, opts);

    auto nDB = 0;
    std::string parameter_string;
    std::tie(szPath, nDB, parameter_string) = _split_path(szPath);
    ...
    if (szType == "tcp") {
        _set_tcp_opts(szPath, opts);
    } else if (szType == "unix") {
        _set_unix_opts(szPath, opts);
    } else {
...
auto ConnectionOptions::_split_uri(const std::string &uri) const
    ...
    auto szType = uri.substr(0, pos);

    auto start = pos + 3;
    pos = uri.find("@", start);
    if (pos == std::string::npos) {
        // No szAuth info.
        return std::make_tuple(szType, std::string{}, uri.substr(start));
    }

    auto szAuth = uri.substr(start, pos - start);

    return std::make_tuple(szType, szAuth, uri.substr(pos + 1));
}

auto ConnectionOptions::_split_path(const std::string &szPath) const
    -> std::tuple<std::string, int, std::string> {
    auto parameter_pos = szPath.rfind("?");
    std::string parameter_string;
    if (parameter_pos != std::string::npos) {
        parameter_string = szPath.substr(parameter_pos + 1);
    }

    auto pos = szPath.rfind("/");
    if (pos != std::string::npos) {
        // Might specified a db number.
        try {
            auto nDB = std::stoi(szPath.substr(pos + 1));

            return std::make_tuple(szPath.substr(0, pos), nDB, parameter_string);
        } catch (const std::exception &) {
    ...
    return std::make_tuple(szPath.substr(0, parameter_pos), 0, parameter_string);
}
...
void ConnectionOptions::_set_tcp_opts(const std::string &szPath, ConnectionOptions &opts) const {
    opts.type = ConnectionType::TCP;

    auto pos = szPath.find(":");
    if (pos != std::string::npos) {
        // Port number specified.
        try {
            opts.port = std::stoi(szPath.substr(pos + 1));
        } catch (const std::exception &) {
            throw Error("invalid URI: invalid port");
        }
    } // else use default port, i.e. 6379.

    opts.host = szPath.substr(0, pos);
}

void ConnectionOptions::_set_unix_opts(const std::string & szPath, ConnectionOptions &opts) const {
...
void Connection::ContextDeleterRedisFree(redisContext* context)
{
    redisFree(context);
}
